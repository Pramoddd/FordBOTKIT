...
app.post('/ford/bot/imageToText',function(req,res){
    console.log(new Date() + 'In imageToText api service');
        ...
        if(error){
            ...
            if (err)
                throw err;
          //  db.collection('errorDetails').findOne({"description":{$regex:".*"+num+".*"}},function(err,result){ 
              db.collection('errorDetails').findOne({"statusCode":parseInt(num[0])},function(err,result){            
                if (err)
                    throw err;
                else{
